...
router.post("/default_board", (req, res) => {
  ...
  con.query(
    `INSERT INTO \`Recipe\` (\`recipe_name\`, \`img_url\`) VALUE('${req.body.recipe_name}', '${req.body.recipeImg}')`,
    (err, result) => {}
  );
  for (var i = 0; i < req.body.board_ingredient.length; i++) {
    con.query(
      `INSERT INTO \`Recipe_Ingredient\` (\`recipe_name\`, \`ingredient\`, \`ratio\`) VALUE('${req.body.recipe_name}', '${req.body.board_ingredient[i][0]}', '${req.body.board_ingredient[i][1]}')`,
      (err, result) => {}
    );
    con.query(
      `INSERT INTO \`Ingredient\` (\`ingredient_name\`, \`count\`, \`ingredient_img_url\`) VALUE('${req.body.board_ingredient[i][0]}', 1, '${req.body.board_ingredient[i][2]}') ON DUPLICATE KEY UPDATE \`count\` = \`count\` + 1;`,
      (err, result) => {}
    );
  }
  con
    .query(
      `INSERT INTO \`Default_Board\` (\`recipe_name\`, \`member_id\`, \`write_time\`, \`text\`, \`snack\`, \`tool\`) VALUE('${req.body.recipe_name}', '${req.body.member_id}', '${write_time}', '${req.body.board_text}', '${snacks}', '${tools}');`,
      (err, result) => {
        if (err) {
...
router.post("/my_board", async (req, res) => {
  let datetime = new Date();
  let write_time =
    datetime.getFullYear() +
    "-" +
  ...
  try {
    await con.query(
      `INSERT INTO \`Recipe\` (\`recipe_name\`, \`img_url\`) VALUE('${
    ...
    for (var i = 0; i < req.body.board_ingredient.length; i++) {
      await con.query(
        `INSERT INTO \`Recipe_Ingredient\` (\`recipe_name\`, \`ingredient\`, \`ratio\`) VALUE('${
          req.body.recipe_name + "_" + req.body.member_id
        }', '${req.body.board_ingredient[i][0]}', '${
          req.body.board_ingredient[i][1]
        }')`
      );
    ...
    await con.query(
      `INSERT INTO \`My_Board\` (\`recipe_name\`, \`member_id\`, \`write_time\`, \`text\`, \`snack\`, \`tool\`) VALUE('${
        req.body.recipe_name + "_" + req.body.member_id
...
router.post("/default_comment", (req, res) => {
  ...
  con
    .query(
      `INSERT INTO \`Default_Board_Comment\` (\`text\`, \`member_id\`, \`datetime\`, \`board_id\`) VALUE(${req.body.text}, ${req.body.member_id}, ${write_time}, ${req.body.board_id})`,
      (err, result) => {
        if (err) {
          res.json({ result: false, error: err });
        } else {
...
router.post("/good_default_board_comment", (req, res) => {
  con.query(
    `INSERT INTO \`DefaultBoardComment_Good\` (\`member_id\`, \`board_id\`) VALUE('${req.body.member_id}, ${req.body.board_id})`,
    (err, result) => {}
  );
  con.query(
    `UPDATE \`Default_Board_Comment\` SET \`good_cnt\` = \`good_cnt\` + 1 WHERE \`board_comment_id\` = ${req.body.board_id}`,
    (err, result) => {}
  );
});
...
router.post("/good_my_board_comment", (req, res) => {
  con.query(
    `INSERT INTO \`MyBoardComment_Good\` (\`member_id\`, \`myboard_comment_id\`) VALUE('${req.body.member_id}, ${req.body.board_id})`,
    (err, result) => {}
  );
  con.query(
    `UPDATE \`My_Board_Comment\` SET \`good_cnt\` = \`good_cnt\` + 1 WHERE \`myboard_comment_id\` = ${req.body.board_id}`,
    (err, result) => {}
  );
});
...
router.post("/good_default_board_cancel", (req, res) => {
  ...
  con.query(
    `UPDATE \`Default_Board\` SET \`good_cnt\` = \`good_cnt\` - 1 WHERE \`board_comment_id\` = ${req.body.board_id}`,
    (err, result) => {}
  );
});
...
router.post("/good_default_board_comment_cancel", (req, res) => {
  con.query(
    `DELETE FROM \`DefaultBoardComment_Good\` WHERE \`member_id\` = '${req.body.member_id}' AND \`board_comment_id\` = '${req.body.board_id}'`,
    (err, result) => {}
  );
  con.query(
    `UPDATE \`Default_Board_Comment\` SET \`good_cnt\` = \`good_cnt\` - 1 WHERE \`board_id\` = ${req.body.board_id}`,
    (err, result) => {}
  );
});
...
router.post("/good_default_board_cancel", (req, res) => {
  con.query(
    `DELETE FROM \`MyBoardComment_Good\` WHERE \`member_id\` = '${req.body.member_id}' AND \`myboard_comment_id\` = '${req.body.board_id}'`,
    (err, result) => {}
  );
  con.query(
    `UPDATE \`My_Board_Comment\` SET \`good_cnt\` = \`good_cnt\` - 1 WHERE \`myboard_comment_id\` = ${req.body.board_id}`,
